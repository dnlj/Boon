...
TEST_CASE("DynamicArray: insert - iterator, move", "[DynamicArray][insert]") {
	GIVEN("an empty DynamicArray") {
		constexpr int value = 32;
		int valueCopy = value;
		Boon::DynamicArray<int> arr;
		...
		WHEN("an item is inserted at the beginning") {
			const auto it = arr.insert(arr.begin(), std::move(valueCopy));
		...
		WHEN("an item is inserted at the end") {
			const auto it = arr.insert(arr.end(), std::move(valueCopy));
		...
		WHEN("an item is inserted past the end") {
			THEN("an error is thrown") {
				REQUIRE_THROWS_AS(arr.insert(arr.end() + 1, std::move(valueCopy)), std::out_of_range);
			}
	...
	GIVEN("an non-empty DynamicArray") {
		...
		WHEN("an item is inserted between the beginning and end") {
			...
			int valueCopy = value;
			constexpr size_t index = 2;
			const auto it = arr.insert(arr.begin() + index, std::move(valueCopy));
		...
		WHEN("an item is inserted past the end") {
			...
			THEN("an error is thrown") {
				REQUIRE_THROWS_AS(
					arr.insert(arr.end() + 1, std::move(valueCopy)),
					std::out_of_range
				);
			}
